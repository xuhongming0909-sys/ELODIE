...
function printHelp() {
  console.log(`ELODIE SKILL

Usage:
  elodie init [target-path] --assistant codex|claude [--force]
  elodie help

Examples:
  elodie init . --assistant codex
  elodie init ./my-project --assistant claude
...
function buildAssistantFile(packageRoot, assistant) {
  ...
  if (assistant === "claude") {
    return { name: "CLAUDE.md", content: source };
  }
...
function initWorkflow(targetPath, assistant, force) {
  ...
  console.log(`Initialized ELODIE SKILL in ${targetRoot}`);
